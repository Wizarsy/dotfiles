...
hooks_symlink() {
  ...
  while IFS= read -r symlink; do
    ...
    cp -v -r --preserve=timestamps --update=older --strip-trailing-slashes "$HOME/$symlink"/* "$SYMLINK_D/$symlink"
    cp -v -r --preserve=timestamps --update=older --strip-trailing-slashes "$SYMLINK_D/$symlink"/* "$HOME/$symlink"
